...
export default function TimelineClipView({
  ...
  const { clips, playheadTime, moveClip, trimClip, selectClip } = useTimelineStore()
  ...
  /** Build snap points from all clip edges + 0 + playhead at drag-start. */
  ...
  function startDrag(e: React.PointerEvent, mode: DragMode) {
    ...
    // Snapshot snap points once so they don't shift mid-drag
    ...
    function onUp(ev: PointerEvent) {
      ...
      const dx = ev.clientX - drag.startX
      const dt = dx / pxPerSec
      if (Math.abs(dx) < 3) return // pure click — no commit

      if (mode === 'move') {
        const newStart = snapTime(Math.max(0, drag.origStart + dt), snapPts)
        if (Math.abs(newStart - drag.origStart) > 0.001) moveClip(clip.id, newStart)
        else { leftMV.set(drag.origStart * pxPerSec) } // snap-back if no change
      } else if (mode === 'trim-left') {
        const rawStart    = Math.max(0, drag.origStart + dt)
        const snapped     = snapTime(rawStart, snapPts)
        const newDuration = drag.origStart + drag.origDuration - snapped
  ...
  return (
    <motion.div
      ...
      // Prevent lane's onClick from deselecting when clicking clip
    ...
    >
      ...
      >
        {/* Bright accent line that appears on hover */}
        <div
      ...
      >
        ...
        {showLabel && (
          <div className="absolute inset-0 flex items-start justify-between px-1.5 pt-1 gap-1 pointer-events-none overflow-hidden">
            ...
            {dispWidth > 80 && (
              <span
                className="text-[9px] font-mono leading-tight shrink-0 opacity-60"
                style={{ color: style.text }}
              >
